...
T["dict_cache()"]["is a DictCache instance"] = function()
  local c = cache.dict_cache()
  expect.equality(type(c), "table")
  expect.equality(c.get ~= nil, true)
  expect.equality(c.set ~= nil, true)
  expect.equality(c.has ~= nil, true)
  expect.equality(c.clear ~= nil, true)
end
...
T["mru_cache()"]["inherits from DictCache"] = function()
  local c = cache.mru_cache(3)
  -- MRUCache should have all DictCache methods
  expect.equality(type(c.get), "function")
  expect.equality(type(c.set), "function")
  expect.equality(type(c.has), "function")
